...
def analyze_network_anomalies(summary_data: List[Dict[str, Any]], grouped_metrics: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    engine_arch = "Unsupervised Scikit-Learn IsolationForest (Real ML)" if HAS_SKLEARN else "Statistical Z-Score & Isolation Heuristics"
        ...
        if len(latencies) >= 3:
            ...
            if HAS_SKLEARN and len(latencies) >= 5:
                try:
                    ...
                    model.fit(X)
                    pred = model.predict([[latest_latency]])[0]  # -1 = Anomalia, 1 = Normal
                    dec_score = model.decision_function([[latest_latency]])[0]
                    iso_score = round(float(-dec_score), 2)  # Inverte para score positivo representar anomalia
            ...
            if is_anomaly_sklearn or z_score >= 2.5 or latest_latency > (mean * 2.0):
                status_anomaly = "Anomalia Crítica (Sklearn IsolationForest)" if is_anomaly_sklearn else "Anomalia Crítica detectada"
