...
const formatTime = (tick) => {
  try {
    return format(
      new Date(tick + (tick.endsWith("Z") ? "" : "Z")),
      "HH:mm:ss",
    );
  } catch {
    return tick;
...
export default function NetOpsDashboard() {
  ...
  const globalStats = useMemo(() => {
    ...
    const totalPings = summaryData.reduce(
      (sum, s) => sum + (s.total_pings || 0),
      0,
    );
    const totalSuccess = summaryData.reduce(
      (sum, s) => sum + (s.successful || 0),
      0,
    );
    const avgLatency =
      summaryData.reduce((sum, s) => sum + (s.avg_latency || 0), 0) /
      summaryData.length;
    const packetLoss =
      summaryData.length > 0
        ? summaryData.reduce(
            (sum, s) => sum + (s.packet_loss_percentage || 0),
            0,
          ) / summaryData.length
        : 0;
    ...
    const csvContent =
      ...
      summaryData
        .map((s) => {
          ...
          const loss =
            s.packet_loss_percentage !== undefined
              ? s.packet_loss_percentage
              : 0;
          const uptime =
            s.total_pings > 0
              ? ((s.successful / s.total_pings) * 100).toFixed(1)
              : 0;
        ...
        <main className="content">
          ...
          >
            <div className="stat-card">
              ...
              <span className="stat-value">
                {globalStats.totalPings.toLocaleString()}
              </span>
            </div>
            <div className="stat-card">
              <div className="stat-header">
            ...
            <div className="stat-card">
              ...
              <span
                ...
                style={{
                  color:
                    parseFloat(globalStats.packetLoss) > 0
                      ? "var(--color-warning)"
                      : "var(--color-healthy)",
                }}
              >
                  ...
                  {viewMode === "grid" ? (
                    <div className="node-grid">
                      <AnimatePresence>
                        {summaryData.map((s) => {
                          const ip = s.target_ip || s.target;
                          const info = IP_INFO[ip] || {
                            label: ip,
                            color: "#06b6d4",
                            icon: <Server size={16} />,
                          };
                          const nodeData =
                            latencyHistoryData[ip] || metricsData[ip] || [];
                          const latest =
                            (metricsData[ip] &&
                              metricsData[ip][metricsData[ip].length - 1]) ||
                            {};
                          const isSuccess =
                            latest.status === "sucesso" ||
                          ...
                          return (
                            ...
                            >
                              ...
                              {(() => {
                                ...
                                const confColor =
                                  insight.confidence_zone > 80
                                    ? "color-healthy"
                                    : insight.confidence_zone > 50
                                      ? "color-warning"
                                      : "color-danger";
                                return (
                                  <div
                    ...
                    <div
                      className="node-table-container"
                      style={{
                            ...
                            return (
                              ...
                              >
                                ...
                                <td style={{ padding: "12px 16px" }}>
                                  ...
                                  >
                                    <div
                                      className={`status-dot ${isSuccess ? "healthy" : "down"}`}
                                      style={{
                                        position: "relative",
                                        top: 0,
                                        left: 0,
                                ...
                                >
                                  <span
                                    style={{
                                      ...
                                      borderRadius: "12px",
                                      background:
                                        s.predictive_risk > 50
                                          ? "rgba(239, 68, 68, 0.15)"
                                          : s.predictive_risk > 20
          ...
          {activeNav === "topology" && (
            ...
            >
              ...
              >
                <Share2 size={22} color="#06b6d4" /> TOPOLOGIA E MAPEAMENTO DE
                REDE
              </h2>
              {(() => {
                if (
                  ...
                  return (
                    <div style={{ color: "#64748b" }}>
                      Topology data loading or not available.
                    </div>
                  );
                }
                ...
                routers.forEach((r, i) => {
                  positions[r.id] = { x: width / 2, y: 50 };
                });
                switches.forEach((s, i) => {
                  positions[s.id] = {
                    x: (width / (switches.length + 1)) * (i + 1),
                    y: 150,
                  };
                });
                servers.forEach((s, i) => {
          ...
          {activeNav === "inventory" && (
            ...
            >
              ...
              >
                ...
                >
                  <Server size={22} color="#06b6d4" /> INVENTÁRIO DE ENDPOINTS
                  MONITORAIS
                </h2>
              ...
              >
                ...
                >
                  <tr>
                    <th style={{ padding: "14px 16px", color: "#64748b" }}>
                      Endpoint / Rótulo
                    </th>
                    <th style={{ padding: "14px 16px", color: "#64748b" }}>
                      Endereço IP / DNS
                ...
                <tbody>
                  {filteredNodes.map((s) => {
                    ...
                    const isSuccess =
                      ...
                      s.last_status === "sucesso";
                    const uptime = (
                      ((s.successful || 0) / (s.total_pings || 1)) *
                      100
                    ).toFixed(1);
                    return (
                      <tr
                            ...
                            style={{
                              ...
                              background:
                                uptime >= 99
                                  ? "rgba(16,185,129,0.15)"
                                  : "rgba(245,158,11,0.15)",
                              color: uptime >= 99 ? "#10b981" : "#f59e0b",
                              fontSize: "12px",
                            }}
                          >
                            {uptime >= 99
                              ? "Em Conformidade (99.9%)"
                              : "Atenção SLA"}
                          </span>
                        </td>
                      </tr>
          ...
          {activeNav === "alerts" && (
            ...
            >
              ...
              >
                <Terminal size={22} color="#ef4444" /> HISTÓRICO DE ALERTAS &
                ANOMALIAS IA
              </h2>
              <div
                style={{
              ...
              >
                {alerts.length === 0 ? (
                  ...
                  >
                    Nenhum alerta crítico registrado recentemente.
                  </div>
                ) : (
                  alerts.map((alert, idx) => (
                    <div
          ...
          {activeNav === "traffic" && (
            ...
            >
              ...
              >
                <Activity size={22} color="#06b6d4" /> ANÁLISE PROFUNDA DE
                TRÁFEGO & LATÊNCIA (PERCENTIS P50 / P90 / P99)
              </h2>
