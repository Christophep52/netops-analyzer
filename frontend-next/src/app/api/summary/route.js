...
async function pingRealIP(ip) {
  try {
    ...
    const success = latency !== null && !stdout.includes('Esgotado') && !stdout.includes('timed out');
...
export async function GET() {
  ...
  const summaryData = pingResults.map(({ ip, success, latency }) => {
    ...
    const avgLatency = hist.latencies.length > 0
      ? parseFloat((hist.latencies.reduce((a, b) => a + b, 0) / hist.latencies.length).toFixed(1))
      : 0;

    const minLatency = hist.latencies.length > 0 ? Math.min(...hist.latencies) : 0;
    const maxLatency = hist.latencies.length > 0 ? Math.max(...hist.latencies) : 0;
