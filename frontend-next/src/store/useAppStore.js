  ...
  alerts: [
    {
      ...
      detail: "Target: Subnet 10.0.4.2 | Action: Blocked",
    },
    {
      time: "14:01:45",
      type: "warning",
      title: "WARNING: HIGH LATENCY SPIKE",
      detail: "Node: AMS-04-A | Latency: 342ms",
    },
    {
      time: "13:58:22",
      type: "info",
      title: "INFO: SYNC_COMPLETE",
      detail: "Registry updated for 14 clusters.",
    },
    {
      time: "13:55:01",
      type: "info",
      title: "INFO: AUTOMATED_BACKUP",
      detail: "Snapshot s-4921-b stored successfully.",
      faded: true,
    },
    {
      time: "13:50:45",
      type: "warning",
      title: "WARNING: FAN_FAILURE_PREDICTED",
      detail: "Chassis 02: Temp rising +2C/min.",
      faded: true,
    },
  ],
  ...
  fetchAll: async () => {
    try {
      const [metricsRes, summaryRes, topologyRes] = await Promise.all([
        ...
        axios
          .get(`${API_URL}/api/topology`, { timeout: 8000 })
          .catch(() => ({ data: { nodes: [], edges: [] } })),
      ]);

      const ips = Object.keys(IP_INFO);
      const historyPromises = ips.map((ip) =>
        axios
          .get(`${API_URL}/api/latency-history?ip=${ip}`, { timeout: 8000 })
          .catch(() => ({ data: { data: [] } })),
      );
      ...
      set({
        ...
        topologyData: topologyRes.data.nodes
          ? topologyRes.data
          : { nodes: [], edges: [] },
      ...
      const mockEdges = [{ source: "core-router", target: "switch-1" }];
      Object.keys(IP_INFO).forEach((ip) => {
        const id = `server-${ip}`;
        mockNodes.push({
          id: id,
          label: `${IP_INFO[ip].label}\n${ip}`,
          type: "server",
        });
      ...
      set({
        ...
        aiInsights: [
          {
            severity: "warning",
            message: "Latency spike detected on AWS us-east-1 node",
            target_ip: "3.218.180.0",
            confidence_zone: 60,
          },
          {
            severity: "info",
            message: "All DNS resolvers responding within normal parameters",
            target_ip: "8.8.8.8",
            confidence_zone: 95,
          },
          {
            severity: "info",
            message: "Network jitter stable across all monitored endpoints",
            target_ip: "1.1.1.1",
            confidence_zone: 92,
          },
        ],
